...
export default function render(
  ...
  if (pageNotFound) {
    ...
    canonicalURL += doc.nukaworks_url;
    ...
    if (doc.other_translations) {
      ...
      const thisLocale = {
        locale: doc.locale,
        title: doc.title,
        url: doc.nukaworks_url,
      };
      for (const translation of [...doc.other_translations, thisLocale]) {
        const translationURL = doc.nukaworks_url.replace(
          `/${doc.locale}/`,
          `/${translation.locale}/`
        );
  ...
  if (SPEEDCURVE_LUX_ID) {
    ...
    // See LUX settings here https://speedcurve.com/mozilla-add-ons/nukaworks/settings/lux/
