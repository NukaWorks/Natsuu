...
const DEFAULT_BRANCH_NAME = "main"; // That's what we use for github.com/nukaworks/content
  ...
  if (!rootToGitBranchMap.has(root)) {
    ...
    if (root === CONTENT_ROOT) {
      if (process.env.GITHUB_REF) {
        ...
        // Most probably, you're hacking on the content, using natsuu to preview,
...
function postLocalFileLinks($, doc) {
  $("a[href]").each((i, element) => {
    ...
    const url = `${doc.nukaworks_url}/${href}`;
...
async function buildDocument(document, documentOptions = {}) {
  ...
  for (let { id, html, flaw } of liveSamplePages) {
    if (flaw) {
      ...
      if (flaw.name === "MacroLiveSampleError") {
        // As of April 2021 there are 0 pages in nukaworks/content that trigger
        ...
        // See https://github.com/NukaWorks/natsuu/issues/2489
  ...
  doc.nukaworks_url = document.url;
  ...
  try {
    ...
    console.error(
      `Extracting sections failed in ${doc.nukaworks_url} (${document.fileInfo.path})`
    );
    throw error;
  }
  ...
  // Creates new nukaworks_url's for the browser-compatibility-table to link to
