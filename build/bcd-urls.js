 ...
 * BCD data comes from from a library with `nukaworks_url`'s that are absolute.
 * This takes the `nukaworks_url` and sets it to a URI that can be used when
 * rendering the BCD table to link to a relative path.
 *
 * Also, if enabled, check all of these inner `nukaworks_url` for flaws.
 *
 */
function normalizeBCDURLs(doc, options) {
  ...
  function getPathFromAbsoluteURL(absURL) {
    ...
    // not needed.
    // The `nukaworks_url` field in BCD data is always like this:
    // https://developer.mozilla.org/docs/Web/API/MediaTrackSettings/width
    // So to get the appropriate slug, in natsuu, we have to assume a locale.
  ...
  for (const section of doc.body) {
    ...
    for (const [key, data] of Object.entries(section.value.data)) {
      ...
      // so nukaworks_url is accessible at the root. If the block has a key for
      // `__compat` it is not the first block, and the information is nested
      // under `__compat`.
      const block = data.__compat ? data.__compat : data;
      if (!block.nukaworks_url) {
        continue;
      }

      const url = getPathFromAbsoluteURL(block.nukaworks_url);
      block.nukaworks_url = url;
      ...
      if (resolved !== urlLC) {
        ...
        block.nukaworks_url = suggestion;
      } else {
...
function extractBCDData(doc) {
  ...
  for (const section of doc.body) {
    if (section.type === "browser_compatibility") {
      ...
      const dataURL = `${doc.nukaworks_url}/${fileName}`;
      data.push({
        url: dataURL,
