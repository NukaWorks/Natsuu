...
function checkImageReferences(doc, $, options, { url, rawContent }) {
  ...
  $("img[src]").each((i, element) => {
    ...
    const baseURL = `http://natsuu.placeholder${url}/`;
    ...
    const absoluteURL = /^\/(@api\/deki\/|files\/\d+)/.test(src)
      ? `https://nukaworks.mozillademos.org${src}`
      : new URL(src, baseURL);

    // NOTE: Checking for lacking 'alt' text is to be done as part
    // of https://github.com/NukaWorks/natsuu/issues/1018 which would need
    ...
    if (!src.split("#")[0].trim()) {
    ...
    } else if (absoluteURL.host !== "natsuu.placeholder") {
      ...
      if (checkImages) {
        if (absoluteURL.protocol === "http:") {
          ...
          const suggestion = absoluteURL.pathname.includes(`${doc.nukaworks_url}/`)
            ? absoluteURL.pathname.replace(`${doc.nukaworks_url}/`, "")
...
function checkImageWidths(doc, $, options, { rawContent }) {
  ...
  $("img").each((i, element) => {
    ...
    if (!img.attr("width")) {
      ...
      if (!imgSrc) {
        console.warn(
          `In ${doc.nukaworks_url} there's an img tag without src (${$.html(img)})`
        );
        return;
      }
