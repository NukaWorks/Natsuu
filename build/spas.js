...
async function buildSPAs(options) {
  ...
  const pullRequestsData = await got(
    "https://api.github.com/search/issues?q=repo:nukaworks/docs-repo+is:pr+is:merged+sort:updated&per_page=10"
  ).json();
    ...
    for (const locale of fs.readdirSync(root)) {
      ...
      const hyData = {
        pullRequestsData: {
          ...
          repo: { name: "nukaworks/docs-repo", url: "https://github.com/NukaWorks/docs-repo" },
        },
        featuredContributor,
      };
      const context = { hyData };
