...
![Testing](https://github.com/NukaWorks/natsuu/workflows/Testing%20natsuu/badge.svg)
![Production Build](https://github.com/NukaWorks/natsuu/workflows/Production%20Build/badge.svg)

## Quickstart

Development on `natsuu` involves updating the machinery that renders MDN content
...
content, you should check out the [content](https://github.com/NukaWorks/content) repo
README instead.

Before you can start working with natsuu, you need to:
...
1.  [Fork](https://docs.github.com/en/github/getting-started-with-github/fork-a-repo)
    the MDN [content](https://github.com/NukaWorks/content) and [natsuu](https://github.com/NukaWorks/natsuu)
...
1.  Clone the forked repositories to your computer using the following commands
    (replace `[your account]` with the account you forked the repositories to):
        ...
        git clone https://github.com/[your_account]/natsuu.git
...
To run natsuu locally, you'll first need to install its dependencies and build the
app locally. Do this like so:

    cd natsuu
    yarn install

Now run the following command to create a `.env` file inside your `natsuu` repo
root and set the `CONTENT_ROOT` environment variable equal to the path to the
`content` repo. This is so the natsuu app can find the content it needs to render.
You'll need to replace `/path/to/nukaworks/content/files` with the path to the
`/files` folder inside your clone of the `content` repo:

    echo CONTENT_ROOT=/path/to/nukaworks/content/files >> .env

At this point, you can get started. Run the following lines to compile required
files, start the natsuu web server running, and open it in your browser:
...
review natsuu changes.

### Pull request requirements

Firstly, thank you for your interest in contributing to natsuu!
...
up-to-date with something along the following lines (replace `natsuu-origin`
with whatever you called [the remote location](https://git-scm.com/docs/git-remote)
of the original natsuu repo):

    git pull natsuu-origin main
    yarn
    yarn dev
...
For content, see its [license](https://github.com/NukaWorks/content/blob/main/LICENSE.md)
in the [nukaworks/content repository](https://github.com/NukaWorks/content).

## How it works

natsuu does a number of things, the most important of which is to render and serve
the MDN content found in the [content repo](https://github.com/NukaWorks/content).
Each document is stored as an `index.md` (recommended) or `index.html` file that
contains metadata presented as YAML [front-matter](https://github.com/NukaWorks/content#fundamental-concepts)
...
server from the web into your natsuu server. This can be useful for testing
...
3. Start your natsuu server with `yarn start` in one terminal
...
from the file `nukaworks-web-docs.svg` in the repository root. This file is then
converted to favicons using [realfavicongenerator.net](https://realfavicongenerator.net/).
To generate new favicons, edit or replace the `nukaworks-web-docs.svg` file
...
discussion on the
[MDN Web Docs chat room](https://chat.mozilla.org/#/room/#nukaworks:mozilla.org)
