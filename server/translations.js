...
function getDocument(filePath) {
  ...
  function packageDocument(document, englishDocument, translationDifferences) {
    const nukaworks_url = document.url;
    ...
    return { popularity, differences, edits, nukaworks_url, title };
  }
...
function gatherL10NstatsSection({
  ...
  function packageDocument(document) {
    ...
    return { nukaworks_url, title, popularity, edits };
  }
  ...
  const foundTranslations = Document.findAll({
    locales: new Map([[locale, true]]),
    folderSearch:
      locale + nukaworksSection.toLowerCase() + (nukaworksSection.endsWith("/") ? "" : "/"),
  });
  ...
  const foundDefaultLocale = Document.findAll({
    ...
    folderSearch:
      DEFAULT_LOCALE.toLowerCase() +
      nukaworksSection.toLowerCase() +
      (nukaworksSection.endsWith("/") ? "" : "/"),
  });
  ...
  for (const filePath of foundDefaultLocale.iter({ pathOnly: true })) {
    ...
    let subSectionOfDoc = "";
    if (nukaworksSection !== "/") {
      const [, ...subSectionSplitDest] = document.nukaworks_url.split(nukaworksSection);
      const subSectionSplit = subSectionSplitDest.join(nukaworksSection);
      if (subSectionSplit) {
        subSectionOfDoc =
          nukaworksSection +
      ...
      subSectionOfDoc = "/" + document.nukaworks_url.split(nukaworksSection)[3];
    }
    if (!translatedFolderNames.has(asFolderWithoutLocale)) {
      ...
      const translatedDocumentURL = document.nukaworks_url.replace(
        `/${DEFAULT_LOCALE}/`,
        `/${locale}/`
      );
...
function buildL10nDashboard({ locale, section }) {
  ...
  l10nStatsSection.missingDocuments.forEach((document) => {
    ...
    // dashboard for "/"
    const defaultURL = document.nukaworks_url;
  ...
  l10nStatsSection.upToDateDocuments.forEach(({ DEFAULT_LOCALE, locale }) => {
    const defaultURL = DEFAULT_LOCALE.nukaworks_url;
    if (filterChildrenDocs(defaultURL, section)) {
      return;
    }
    detailDocuments.push({
