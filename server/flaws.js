...
function packageDocument(doc) {
  const { modified, nukaworks_url, title } = doc;
  const popularity = {
  ...
  return { popularity, flaws, modified, nukaworks_url, title };
}
...
module.exports = (req, res) => {
  ...
  for (const filePath of glob.sync(
    ...
    if (
      ...
      (filters.nukaworks_url &&
        !doc.nukaworks_url.toLowerCase().includes(filters.nukaworks_url.toLowerCase())) ||
      (filters.title &&
  ...
  documents.sort((a, b) => {
    switch (sortBy) {
      ...
      case "nukaworks_url":
        if (a.nukaworks_url.toLowerCase() < b.nukaworks_url.toLowerCase()) {
          return sortMultiplier * -1;
        } else if (a.nukaworks_url.toLowerCase() > b.nukaworks_url.toLowerCase()) {
