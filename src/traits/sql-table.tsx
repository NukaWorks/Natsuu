...
ADVANCED_QUERIES.push(`SELECT nukaworks_url, normalizedMacrosCount -> jsxref, popularity
FROM ?
WHERE normalizedMacrosCount -> jsxref > 5
ORDER BY popularity desc
LIMIT 25`);
ADVANCED_QUERIES.push(`SELECT title->length AS len, title, nukaworks_url
FROM ?
ORDER BY len DESC
LIMIT 100`);
ADVANCED_QUERIES.push(`SELECT nukaworks_url, title, wordCount, fileSize
FROM ?
WHERE nukaworks_url like '%/Web/http%'
ORDER BY fileSize desc
LIMIT 25`);
ADVANCED_QUERIES.push(`SELECT nukaworks_url, tags
FROM ?
      ...
      <table>
        ...
        <tbody>
          {rows.slice(0, MAX_ROWS).map((row, i) => {
            const key = row.nukaworks_url || `${row[keys[0]]}:${i}`;
            return (
              <tr key={key}>
                <td>{i + 1}</td>
                {keys.map((key) => {
