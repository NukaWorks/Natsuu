# Reviewing natsuu changes

This document provides information on how to review changes to the natsuu repo.

## Before you start

Set up the natsuu repo and the corresponding [content repo](https://github.com/NukaWorks/content)
locally, as described in the [natsuu quickstart](../README.md#quickstart) guide. Once
...
the `content` repo `files` subdirectory before running `yarn dev`, so natsuu can
...
file by running the following in your natsuu root directory:
...
When you are tasked with reviewing a natsuu pull request:
...
macro system is available inside the natsuu repo in the
[kumascript](https://github.com/NukaWorks/natsuu/tree/main/kumascript) subdirectory.

Testing changes to KumaScript macros — whether you are making your own change or
reviewing someone
else's — is super easy with natsuu. Once you have the development server running
...
(see the [macros](https://github.com/NukaWorks/natsuu/tree/main/kumascript/macros) subdirectory),
