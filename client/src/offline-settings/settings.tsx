...
function Settings() {
  ...
  // Workaround to avoid "Error: Too many re-renders." (https://github.com/NukaWorks/natsuu/pull/5744).
  const updateTriggered = useRef(false);

  useEffect(() => {
    const init = async () => {
      setSettings(await window.nukaworksWorker.offlineSettings());
      setStatus(await window.nukaworksWorker.updateAvailable());
  ...
  const updateSettings = async (change: SettingsData) => {
    setSaving(true);
    let newSettings = await window.nukaworksWorker.setOfflineSettings(change);
    setSettings(newSettings);
  };

  useInterval(() => {
    const next = window.nukaworksWorker.status();
