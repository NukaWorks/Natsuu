...
export class MDNWorker {
  ...
  async messageHandler(event) {
    switch (event.data.type) {
      case "updateStatus":
        await window.nukaworksWorker.setStatus(event.data);
        break;
      case "pong":
        console.log("pong");
        break;
...
declare global {
  interface Window {
    nukaworksWorker: MDNWorker;
  }
}

if (!window.nukaworksWorker) {
  window.nukaworksWorker = new MDNWorker();
}

function registerMessageHandler() {
  navigator.serviceWorker.addEventListener(
    "message",
    window.nukaworksWorker.messageHandler
  );
}

if (window.nukaworksWorker.settings.offline) {
  window.nukaworksWorker.enableServiceWorker(window.nukaworksWorker.settings.preferOnline);
}
