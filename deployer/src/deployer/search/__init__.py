...
def to_search(file, _index=None):
    ...
    if "doc" not in data:
        ...
        return
    doc = data["doc"]
    locale, slug = doc["nukaworks_url"].split("/docs/", 1)
    if slug.endswith("/Index"):
        ...
        # See also https://github.com/NukaWorks/natsuu/issues/1786
        return
    ...
    return Document(
        ...
        # natsuu displays search results, it doesn't use this `slug` value to
        # make the URLs in the search results listings. It's using the `nukaworks_url`
        # for that.
