...
def test_analyze_pr_prefix():
    doc = {"doc": {"nukaworks_url": "/en-US/docs/Foo"}}
    with mock_build_directory(doc) as build_directory:
        ...
        assert "- <https://pr007.content.dev.nukaworks.mozit.cloud/en-US/docs/Foo>" in comment


def test_analyze_pr_flaws():
    doc = {
        "doc": {
            "nukaworks_url": "/en-US/docs/Foo",
            "title": "Foo",
            "flaws": {
                "faux_pas": [
...
def test_get_patch_lines_basic():
    ...
    assert "Hi Ryan" in lines["files/en-us/nukaworks/kitchensink/index.html"]
