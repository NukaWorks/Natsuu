...
def post_about_deployment(build_directory: Path, **config):
    ...
    for doc in get_built_docs(build_directory):
        url = nukaworks_url_to_dev_url(config["prefix"], doc["nukaworks_url"])
        links.append(f"- <{url}>")
    links.sort()
...
def nukaworks_url_to_dev_url(prefix, nukaworks_url):
    template = "https://{prefix}.content.dev.nukaworks.mozit.cloud{nukaworks_url}"
    return template.format(prefix=prefix, nukaworks_url=nukaworks_url)
...
def post_about_dangerous_content(
    ...
    if comments:
        ...
        for doc, comment in comments:
            ...
            if config["prefix"]:
                url = nukaworks_url_to_dev_url(config["prefix"], doc["nukaworks_url"])
                lines.append(f"URL: [`{doc['nukaworks_url']}`]({url})")
            else:
                lines.append(f"URL: `{doc['nukaworks_url']}`")
...
def get_patch_lines(patch: PatchSet):
    ...
    for patched_file in patch:
        ...
        # so for example: `files/en-us/nukaworks/kitchensink/index.html`
