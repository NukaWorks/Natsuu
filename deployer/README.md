...
The natsuu Deployer does two things. First, it's used to upload document
...
upload into: `nukaworks-content-dev` (for variations or experimental versions of
the site), `nukaworks-content-stage`, and `nukaworks-content-prod`.
...
most likely what you'll want for uploads to the `nukaworks-content-stage` and
`nukaworks-content-prod` S3 buckets. However, for uploads to the `nukaworks-content-dev`
...
cd deployer
poetry run deployer upload --bucket nukaworks-content-dev --prefix pr1234 ../client/build
```

```sh
export CONTENT_ROOT=/path/to/content/files
export CONTENT_TRANSLATED_ROOT=/path/to/translated-content/files
export DEPLOYER_BUCKET_NAME=nukaworks-content-dev
...
E.g. `nukaworks_docs_20210331093714`. And then there's an alias with a more "generic" name.
E.g. `nukaworks_docs`. It's the alias name that Kuma uses to send search queries to.
...
- Yesterday: index `nukaworks_docs_20210330093714` and `nukaworks_docs --> nukaworks_docs_20210330093714`
- Today:
  - create new index `nukaworks_docs_20210331094500`
  - populate `nukaworks_docs_20210331094500` (could take a long time)
  - atomically re-assign alias `nukaworks_docs --> nukaworks_docs_20210331094500` and delete old index `nukaworks_docs_20210330093714`
  - delete old index `nukaworks_docs_20210330`
...
The `prefix` is used to specify the proper Dev subdomain (`{prefix}.content.dev.nukaworks.mozit.cloud`) for the URLs of the built documents. For example,
if `--prefix experiment1` is specified, it will list:

```md
## Preview URLs

- <https://experiment1.content.dev.nukaworks.mozit.cloud/en-US/docs/MDN/Kitchensink>
```

...assuming the only page that was built was `build/en-us/docs/nukaworks/kitchensink`.
Note that this assumes the PR build has been deployed to the Dev server.

### `--repo`

This is useful for debugging when the PR you made wasn't on `nukaworks/content`. For example:
...
This is needed to be able to find the PR (on <https://github.com/NukaWorks/content/pulls>)
...
like <https://github.com/NukaWorks/content/pull/3381/checks?check_run_id=2169672013> for
...
- `DEPLOYER_BUCKET_NAME` is equivalent to using `--bucket` (the
  default is `nukaworks-content-dev`)
